...
def execute(cmd):
    ...
    #srderr- error output stream>>> STDOUT- this is output stream(execute)>>>result working
...
class NetCat:
    ...
    def listen(self):
        self.socket.bind((self.args.target, self.args.port)) #listening side
...
if __name__ == "__main__":
    #creating a command line interface>>>ArgumentParser- pulling arguments>>>#formatter_class- it can be used to display a help message as a single block of text
    ...
    parser.add_argument('-t', '--target', default='	95.25.210.54', help='specidied IP')  # sets the IP
    ...
    nc = NetCat(args, buffer=None)
    nc.run()
#КОД НЕ ИСПРАВЛЕН. ОТСТУПЫ СДЕЛАЛ НА МЕТОДАХ
